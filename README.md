...
| 模块 | 入口 | 内容 |
|---|---|---|
| **学习资料** | `materials.html` | N5 → N1 的基础知识・语法・词汇，共 168 个单元页；日语内容配振假名注音，每单元附自测 |
| **真题练习** | `exam/index.html` | 题型专练（按子题型 / 按真题顺序取题）＋ 整卷仿真（155 分钟、听力放最后、只播一次）；已入库 N2 2023.7 / 2023.12 两卷 |
| **课堂单词** | `jp-vocab/index.html` | 按课堂主题打包的副词词卡，4 单元 / 63 张卡；浏览、翻转卡、测验、间隔重复复习四态，词与例文均配音频 |

## 目录结构

```
├── index.html              ← 门户首页（三模块入口）
├── materials.html          ← 学习资料落地页（168 单元索引）
├── assets/
│   ├── style.css           ← 全站样式与设计令牌
...
├── jp-vocab/               ← 课堂单词：vocab.css + vocab.js + 每单元一个子目录
...
任何浏览器直接打开 `index.html` 即可，无构建步骤、无依赖。

用 `file://` 打开时，少数依赖 `fetch` 的附属统计（如课堂单词目录页的每单元词数）会被静默省略，其余功能不受影响。
